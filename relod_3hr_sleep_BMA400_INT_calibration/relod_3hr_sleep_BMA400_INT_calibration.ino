...
#define CURRENT_FIRMWARE_VERSION "3.0"

// Buffer for storing/staging the raw Xtalk profile:
static constexpr size_t XTALK_DATA_SIZE = VL53L5CX_XTALK_BUFFER_SIZE;
uint8_t           xtalkData[XTALK_DATA_SIZE];

//— calibration parameters (ST’s “Table 1” suggests these defaults) ——
...
//— how big is the raw crosstalk data in bytes?  
//  your plugin header or docs should give you this.  
...
String getFirmwareUpdateUrl() {
  ...
  if (httpCode == 200) {
    ...
    Serial.println("🔧 Received firmware info: " + payload);

    StaticJsonDocument<512> doc;
    DeserializationError error = deserializeJson(doc, payload);
    if (!error) {
      const char* latestVersion = doc["version"];
      const char* signedUrl = doc["url"];

      if (String(latestVersion) != CURRENT_FIRMWARE_VERSION) {
        Serial.println("🚀 New firmware available: " + String(latestVersion));
        firmwareUrl = String(signedUrl);
      } else {
    ...
    Serial.printf("❌ Firmware check HTTP error: %d\n", httpCode);
  }
...
bool performOTA(String firmwareUrl) {
  ...
  client.setInsecure();  // Use only if S3 certs are trusted

  HTTPClient http;
  Serial.println("⬇️  Starting OTA from: " + firmwareUrl);
  ...
  if (httpCode == 200) {
    ...
    if (canBegin) {
      ...
      if (written == contentLength) {
        ...
        Serial.printf("❌ Only %d/%d bytes written. Abort.\n", written, contentLength);
        return false;
      }
      ...
      if (Update.end()) {
        ...
        Serial.printf("❌ Update.end() error #%d\n", Update.getError());
      }
    } else {
...
void setup() {
  ...
  delay(200);                // give the USB‐serial a moment
  ...
  if (updateUrl != "") {
    performOTA(updateUrl);  // This will reboot if OTA succeeds
  }
...
void loop() {
  ...
  delay(100); // Short delay to allow the sensor to wake up
  processRangingData();
  Serial.println("Out of process ranging data loop");
...
void initSensors() {
  ...
  // 1) Bring up I²C at 400 kHz
  Wire.begin();
  Wire.setClock(400000);
  Serial.println(F(">> I²C at 400 kHz"));

  // 2) Initialize the VL53L5CX
  if (!myImager.begin(0x29, Wire)) {
    Serial.println(F("❌ VL53L5CX not found!"));
    while (1) delay(500);
  }
  Serial.println(F("✔ VL53L5CX begin() succeeded"));

  if (!myImager.isConnected()) {
    Serial.println(F("❌ VL53L5CX isConnected() failed!"));
    while (1) delay(500);
  }
  Serial.println(F("✔ VL53L5CX isConnected() OK"));

  // 3) Wake the sensor (in case it was sleeping)
  myImager.setPowerMode(SF_VL53L5CX_POWER_MODE::WAKEUP);
  delay(10);
  Serial.println(F("✔ VL53L5CX is awake"));

  // 4) Force full 8×8 resolution
  myImager.setResolution(8 * 8);
  Serial.println(F("▶︎ Resolution set to 8×8"));

  // 5) Start continuous ranging
  if (myImager.startRanging()) {
    Serial.println(F("▶︎ Ranging started"));
  } else {
