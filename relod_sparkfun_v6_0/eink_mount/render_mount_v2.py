...
OUT=ROOT/'output_v2'
cfg=json.loads((OUT/'dimensions_and_checks_v2.json').read_text())
...
lid=read(OUT/'LidTop_measured_panel_mount_v2.stl')
added=read(ROOT/'added_features_v2.stl')
rear=read(ROOT/'rear_assembled_v2.stl')
panel=read(ROOT/'panel_reference_v2.stl')
...
d.text((60,32),'Revised mount - measured screen outline',font=font(38,True),fill='#142b3e')
d.text((60,88),'Panel: 79.05 x 36.71 x 0.95 mm | Revision 2 | Prototype for fit testing',font=font(22),fill='#506576')
...
legend=[('#1ea5a3','Wider guides, border supports and screw bosses'),('#e6b54d','Glass panel reference - not a printed part'),('#316bab','Separate rear retainer with open cable end')]
for i,(color,label) in enumerate(legend):
    y=976+i*36
    d.rounded_rectangle((62,y+3,82,y+23),radius=3,fill=color)
    d.text((96,y-3),label,font=font(21),fill='#233e50')
d.text((1060,976),'Pocket: 80.05 x 37.71 mm',font=font(22),fill='#233e50')
d.text((1060,1012),'Original slot retained: 55 x 31.9 mm',font=font(22),fill='#233e50')
d.text((1060,1048),'Ribbon exits toward the bottom.',font=font(22),fill='#233e50')
canvas.save(OUT/'mount_preview_v2.png')

# Full-size close-up is useful for reviewing the retaining geometry.
coupon=read(OUT/'Mount_fit_coupon_v2.stl')
render([(coupon,(30,165,163),0)],(1200,1000),az=-65,el=55).save(OUT/'fit_coupon_preview_v2.png')
